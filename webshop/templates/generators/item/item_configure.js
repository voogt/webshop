class ItemConfigure {
	...
	show_configure_dialog() {
		...
		this.dialog.show();
		var cached = JSON.parse(localStorage.getItem(this.get_cache_key()))
		this.dialog.set_values(cached);
		this.on_attribute_load(cached)

		$(".btn-configure").prop("disabled", false);
	}

	on_attribute_load(values) {
	...
	show_range_input_if_applicable(fieldname) {
		...
		console.log("changed_field.get_value", changed_field)
